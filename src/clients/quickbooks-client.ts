...
export class QuickbooksClient {
  ...
  private saveTokensToEnv(): void {
    // A host application may run many per-user MCP subprocesses concurrently.
    // Give each process an isolated token-output file instead of sharing the
    // repository .env (which could leak/overwrite another user's rotation).
    const tokenPath = process.env.QUICKBOOKS_TOKEN_OUTPUT_PATH
      ? path.resolve(process.env.QUICKBOOKS_TOKEN_OUTPUT_PATH)
